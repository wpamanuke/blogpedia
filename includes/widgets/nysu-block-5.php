...
class blogpedia_block_5_widget extends WP_Widget {
   function __construct() {
		parent::__construct(
			'blogpedia_block_5', esc_html_x('BLOGPEDIA Block 5 - Mag (8/12)', 'widget name', 'blogpedia'),
			array(
				'classname' => 'blogpedia-block-5-widget',
				'description' => esc_html__('BLOGPEDIA Block 5 widget posts (horizontal big small)', 'blogpedia'),
				...
				echo '<div class="blogpedia-normal-widget">' . "\n";
					...
					while ($widget_posts->have_posts()) : $widget_posts->the_post();
						...
						if ($i==1) { 
							...
							get_template_part( 'template-parts/block/block', '2' );
							echo '</div>';
						} else {
							if ($i==2) {
								echo '<div class="col-md-6">';
								}
							get_template_part( 'template-parts/block/block', '3' );
							if ($i==$last) {
								echo '</div>';
								}
						}
    ...
    function form($instance) {
		...
		<div class="blogpedia-border">
			<img src="<?php echo get_template_directory_uri() . '/public/images/widget/block-5.png'; ?>" alt="" />
		</div>
        <p>
