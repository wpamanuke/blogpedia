...
class blogpedia_slider_1_widget extends WP_Widget {
   function __construct() {
		parent::__construct(
			'blogpedia_slider_1', esc_html_x('BLOGPEDIA Slider 1 - Mag (12/12)', 'widget name', 'blogpedia'),
			array(
				'classname' => 'blogpedia-slider-1-widget',
				'description' => esc_html__('BLOGPEDIA Slider 1 widget posts  (1 by 1)', 'blogpedia'),
				'customize_selective_refresh' => true
			)
		);
	}
	function widget($args, $instance) {
		$defaults = array('title' => '', 'category' => 0, 'tags' => '', 'postcount' => 8, 'offset' => 0, 'sticky' => 1);
        ...
        echo $args['before_widget'];
			if ($widget_posts->have_posts()) :
				...
				echo '<div class="owl-carousel blogpedia-slider-1">' . "\n";
					$i = 0;
					while ($widget_posts->have_posts()) : $widget_posts->the_post();
						get_template_part( 'template-parts/block/slider', '1' );
					endwhile;
					wp_reset_postdata();
				echo '</div>' . "\n";
				echo '</div>' . "\n";
    ...
    function form($instance) {
        $defaults = array('title' => '', 'category' => 0, 'tags' => '', 'postcount' => 8, 'offset' => 0, 'sticky' => 1);
        $instance = wp_parse_args($instance, $defaults); ?>
		<div style="margin:5px 0;border:2px solid #ff0000;">
			<img src="<?php echo get_template_directory_uri() . '/public/images/widget/slider-1.png'; ?>" alt="" />
		</div>
        <p>
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('postcount')); ?>"><?php esc_html_e('Post Count (max. 50):', 'blogpedia'); ?></label>
	    ...
	    <p>
        	<label for="<?php echo esc_attr($this->get_field_id('offset')); ?>"><?php esc_html_e('Skip Posts (max. 50):', 'blogpedia'); ?></label>
